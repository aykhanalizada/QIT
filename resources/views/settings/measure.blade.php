...
@section('content')
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mb-3">
        <table class="w-full text-sm text-lefrt tl:text-right text-gray-500 dark:text-gray-400">
            ...
            <th scope="col" colspan="2" class="px-6 py-3">
                Ölçü Vahidlərinin siyahısı
            </th>

            <th scope="col" colspan="3" class="px-6 py-3 text-end">
                <span id="createBtn" class="bg-green-600 p-1.5 rounded cursor-pointer" onclick="createMeasure()">Ölçü Vahidi Əlavə Et
            ...
            @foreach($measures as $measure)
                ...
                <tr id="userRow_{{$measure->id}}"
                    ...
                    <td class="px-6 py-4 text-center">
                        <span id="showUser" onclick="showMeasure({{json_encode($measure)}})"
                        ...
                        <span onclick="editMeasure({{json_encode($measure)}})"
                              id="editUser"
                              class="cursor-pointer font-medium text-blue-600 dark:text-blue-500 hover:underline"><i
                                class="fas fa-edit text-xl pr-2"></i></span>

                        <span
                            onclick="deleteMeasure( {{$measure->id}} ,'{{route('deleteMeasure',['id'=>$measure->id] ) }}'  ) "
    ...
    <x-measures.create-measure/>
    <x-measures.show-measure :measures="$measures"/>
    <x-measures.edit-measure :measures="$measures"/>
...
@section('bottomScript')
    <script src="{{asset('js/measure/measurePage.js')}}"></script>
    <script src="{{asset('js/measure/measureAction.js')}}"></script>
@endsection
